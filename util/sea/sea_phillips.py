...
def phillips_spectrum(N=32, L=64, wind_speed=32.0, wind_direction=(1.0, 0.0), wave_amp=0.0002, gravity=9.81, damping=0.001):
    """
    生成 Phillips 光谱波浪数据，包括方向、角速度、振幅和初始相位（数据量减半）。
    ...
    """
    wind_dir = np.array(wind_direction) / np.linalg.norm(wind_direction)
    L_wind = (wind_speed ** 2) / gravity
    l2 = L_wind ** 2 * damping ** 2
    ...
    for m_prime in range(N // 2):  # 只计算半个波矢量空间
        for n_prime in range(N):
            ...
            k_unit = k / k_length
            k_dot_wind = np.dot(k_unit, wind_dir)
            ...
            # 保存数据
            freqDir.append((round(k_unit[0], 6), round(k_unit[1], 6)))
            freqOmega.append(round(np.sqrt(gravity * k_length), 6))
            freqAmp.append(round(np.sqrt(phillips_value), 6))
            freqPhaseOffset.append(round(np.random.uniform(0.0, 2 * np.pi), 6))
